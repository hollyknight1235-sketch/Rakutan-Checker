...
self.addEventListener('fetch', e => {
  ...
  // v2.5.0修正: 旧コードは './'→'' の endsWith('') が常にtrueになり、
  // Firebase等の全リクエストまで捕捉・キャッシュしていた。自オリジンの
  // アプリ資産のみに限定する。
  const isAsset = url.origin === self.location.origin && (
    url.pathname.endsWith('/') ||
    ['index.html','manifest.json','icon.png'].some(n => url.pathname.endsWith('/' + n))
  );
  if (isAsset) {
    e.respondWith(
